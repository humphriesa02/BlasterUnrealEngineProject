...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World)
	{
		...
		else // Didn't hit head, check rest of boxes
		{
...
void ULagCompensationComponent::SaveFramePackage()
{
	...
	// For the first two frame packages, we simply add them to the frame history 
	...
	// Once we have two or more frame packages,
	else
	{
		// Check history length, if it's greater than max record time,
		float HistoryLength = FrameHistory.GetHead()->GetValue().Time - FrameHistory.GetTail()->GetValue().Time;
		while (HistoryLength > MaxRecordTime)
		{
			// remove nodes from the tail until history length is less than max record time.
			FrameHistory.RemoveNode(FrameHistory.GetTail());
			HistoryLength = FrameHistory.GetHead()->GetValue().Time - FrameHistory.GetTail()->GetValue().Time;
		}
		// When we've removed enough nodes, save the current frame package.
		FFramePackage ThisFrame;
		SaveFramePackage(ThisFrame);
		FrameHistory.AddHead(ThisFrame);
